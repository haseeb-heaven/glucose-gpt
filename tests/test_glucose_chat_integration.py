#!/usr/bin/env python3
"""
Integration tests for glucose_chat.py using mocks for external dependencies
"""
import os
import sys
import tempfile
...
def test_configuration_loader_integration():
    """Test ConfigurationLoader with mocked dependencies."""
...
""")
    ...
    try:
        # Mock streamlit and config manager
        with mock.patch('streamlit.error') as mock_st_error, \
             mock.patch('streamlit.info') as mock_st_info, \
             mock.patch('glucosegpt.examples.glucose_chat.config_manager') as mock_config_manager:
            ...
            mock_config_manager.get_configuration_sources.return_value = {
                'env_file': 'Found',
                'streamlit_secrets': 'Available',
                'pyproject_toml': 'Found',
                'project_toml': 'Found'
            }
            
            # Import and test
            from glucosegpt.examples.glucose_chat import ConfigurationLoader
            
            config_loader = ConfigurationLoader()
            result = config_loader.load_defaults()
            ...
            print("✅ ConfigurationLoader integration test passed")
...
def test_complete_workflow_integration():
    """Test a complete workflow from configuration loading to data analysis."""
    ...
    with mock.patch('glucosegpt.examples.glucose_chat.file_log') as mock_logger, \
        ...
        from glucosegpt.examples.glucose_chat import (
            ConfigurationLoader, LibreClientManager, DataFormatter, AIAnalyzer
        )
...
def run_integration_tests():
    ...
    try:
        test_configuration_loader_integration()
